...
class eDFDToGraphTransformation {
	...
	val eDFDNodeElementRule = createRule.precondition(NodeElements.Matcher.querySpecification).action [
		...
		val subgraph = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null, eDFD as NamedEntity).filter(
			Subgraphs).head
	...
	val eDFDNodeAttackerZoneRule = createRule.precondition(Boundaries.Matcher.querySpecification).action [
		...
		// find subgraph in target model
		val subgraph = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null, it.edfd as NamedEntity).filter(
			Subgraphs).head

		for (Element el : eDFDBoundary.elements) {
			// find this node in target graph
			var locate_correct_graph_node = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null,
				el as NamedEntity).filter(Node).head
	...
	val eDFDOutFlowRule = createRule.precondition(NodeElements.Matcher.querySpecification).action [
		...
		var locate_correct_graph_node = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null,
			eDFDElement as NamedEntity).filter(Node).head
		for (Node n : subgraph.nodes) {
			if (n.name == locate_correct_graph_node.name) {
				locate_correct_graph_node = n
			}
		...
		for (Flow fl : eDFDOutgoingFlows) {
			...
			for (Asset edfdasset : fl.assets) {
				graphassets.add(
					engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null, edfdasset as NamedEntity).filter(
						GraphAsset).head)
			}
	...
	val eDFDProcessResponsibilitiesRule = createRule.precondition(Responsibilities.Matcher.querySpecification).action [
		...
		val node_of_process = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null,
			eDFDResponsibilityProcess).filter(Node).head
		...
		for (Asset s : eDFDIncomingAssets) {
			...
			incomingassets_of_process.add(
				engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null, o).filter(GraphAsset).head)
		}
		...
		val graphResponsibility = node_of_process.createChild(node_Responsibility,
			nodeResponsibility) as NodeResponsibility => [
			// set incoming assets
		...
		graphResponsibility.ID = eDFDResponsibilityProcess.name.concat(actionsString).concat(
			eDFDResponsibility.incomeassets.toString)
	...
	val eDFDProcessRule = createRule.precondition(ProcessElements.Matcher.querySpecification).action [
		...
		// find target process node (in graph)
		val node_of_process = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null, eDFDProcess).filter(Node).
			head
		...
		for (ContractBase r : eDFDProcessResponsibilities) {
			...
			responsibilities_of_process.add(
				engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null, o).filter(NodeResponsibility).head)
		}
	...
	val eDFDTBRule = createRule.precondition(BoundariesAssets.Matcher.querySpecification).action [
		...
		val asset = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null, eDFDAsset as NamedEntity).filter(
			GraphAsset).head
		...
		for (Element e : eDFDElementsInBoundary) {
			var locate_correct_graph_node = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null,
				e as NamedEntity).filter(Node).head
			for (Node n : subgraph.nodes) {
				if (n.name == locate_correct_graph_node.name) {
					locate_correct_graph_node = n
				}
	...
	val eDFDModifyAsset = createRule.precondition(edfdtosimplegraph.EDFDAsset.Matcher.querySpecification).action [
		...
		var locate_correct_graph_node = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null,
			eDFDAssetSource as NamedEntity).filter(Node).head
		for (Node n : subgraph.nodes) {
			if (n.name == locate_correct_graph_node.name) {
				locate_correct_graph_node = n
			}
		...
		for (Element e : eDFDAssetTargets) {
			locate_correct_graph_node = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null,
				e as NamedEntity).filter(Node).head
			for (Node n : subgraph.nodes) {
				if (n.name == locate_correct_graph_node.name) {
					locate_correct_graph_node = n
	...
	val propagateLabelsInOrder = createRule.precondition(edfdtosimplegraph.EDFD.Matcher.querySpecification).action [
		...
		for (Element f : allFlows) {
			...
			val outgoing = engine.edfd2simplegraph.getAllValuesOfgraphElements(null, null, f as NamedEntity).
				filter(Edge).head
			...
			for (NodeResponsibility nr : node.responsibility) {
				...
				var pMax = getMax(nr.incomingassets)
				...
				if (!outgoing.edgelabel.exists[l|l.objective == "Privacy"]) {
					...
					switch contractTypeStr {
						...
						case "[Clustering]": {
							...
							val allHaveLabels = !nr.incomingassets.empty && nr.incomingassets.forall [ ina |
								levelOf(ina.assetlabel, "Privacy") >= 0
							]
							val allPrivacyLabelsAreN = allHaveLabels && !nr.incomingassets.exists [ ina |
								levelOf(ina.assetlabel, "Privacy") != 0
							]
						...
						case "[Decrypt]": {
							...
							if (!nr.incomingassets.empty) {
								for (var i = 0; i < nr.incomingassets.size; i++) {
									...
									val outgoingAssetIndex = if(i < nr.outgoingassets.size) i else nr.outgoingassets.
											size - 1
									val correspondingOutgoingAsset = if(outgoingAssetIndex >= 0) nr.outgoingassets.get(
											outgoingAssetIndex) else null
									if (correspondingOutgoingAsset !== null &&
										outgoing.graphassets.contains(correspondingOutgoingAsset)) {
										for (o : Objective.values) {
											val oStr = o.literal
											if (!outgoing.edgelabel.exists[l|l.objective == oStr]) {
						...
						case "[Splitter]": {
							...
							if (!nr.incomingassets.empty) {
								val matchingOutgoingAsset = nr.outgoingassets.findFirst [ outAsset |
									outgoing.graphassets.contains(outAsset)
								]
								if (matchingOutgoingAsset !== null) {
									val outgoingAssetIndex = nr.outgoingassets.indexOf(matchingOutgoingAsset)
									val incomingAssetIndex = if(outgoingAssetIndex <
											nr.incomingassets.size) outgoingAssetIndex else nr.incomingassets.size - 1
	...
	def int getMax(EList<GraphAsset> assets) {
		// Berechne p_max für alle Contracts, die Input-Assets benötigen
	...
	def int getMaxLevel(EList<Asset> assets) {
		// Berechne p_max für alle Contracts, die Input-Assets benötigen
		var pMax = -1 // Initialize with -1 (no label)
		for (ina : assets) {
			val level = levelOfValue(ina.value,"Privacy")
			if (level >= 0) { // Only consider actual labels (level >= 0)
				pMax = Math::max(pMax, level)
			}
		}
	...
	def void logEdge(Edge e) {
		...
		/* 2️   labels */
		val lbls = e.edgelabel.filter[level >= 0].sortBy[objective].map[o|o.objective + "=" + o.level]
		sb.append("  ").append(
			lbls.empty ? "[no-label]" : lbls.join(", ")
		)

		/* 3️   originating operations (Encrypt, Joiner …)             */
		val ops = e.source.responsibility.filter[r|r.outgoingassets.exists[e.graphassets.contains(it)]].map [
